...
Wide events via `onEvent` remain opt-in for harnesses and debugging. When set, `onEventCategory` defaults to `"ops"` (queue/shed/drop/elide/finalize/persist/`observation_dropped`) so subscribers are not flooded with per-request `observed` chatter; pass `"all"` to include `observed` and `context_missing`. With no `onEvent`, the recorder skips wide-event emit work entirely — counters still update via `stats()`. Exporters do **not** need `onEvent` (or `"all"` event verbosity). `pressureStats()` is a deprecated alias of `stats()`.
