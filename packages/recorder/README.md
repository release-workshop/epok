...
Wide events via `onEvent` remain opt-in for recorder ops (shed, drop, elide, finalize, persist, `observation_dropped`, `context_missing`). Per-request HTTP facts use `hooks` (`onInbound` / `onDependency` / `onResponse`); the host maps those to whatever sink it uses. With no `onEvent`, the recorder skips wide-event emit work entirely — counters still update via `stats()`. Exporters do **not** need `onEvent`. `pressureStats()` is a deprecated alias of `stats()`.
